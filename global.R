...
# create a column to flag if inflation or gdp dont have enough obversavtions to scale
scale_data_list <- list()
for(i in 1:length(country_names)){
  country_name <- country_names[i]
  sub_dat <- scale_data %>% filter(Country == country_name)
  sub_dat$inflation_ok <- length(which(is.na(sub_dat$Inflation.Index))) <= 1
  sub_dat$gdp_ok <- length(which(is.na(sub_dat$GDP))) <= 1
